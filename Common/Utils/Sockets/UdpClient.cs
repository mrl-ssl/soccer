...
namespace MRL.SSL.Common.Utils.Sockets
{
    ...
    public class UdpClient : IDisposable
    {
        ...
        /// <summary>
        /// On data sent event handler
        /// </summary>
        public event EventHandler<UdpSentEventArgs> Sent;
        /// <summary>
        ...
        /// <summary>
        /// On join multicast group event handler
        /// </summary>
        public event EventHandler<IPAddress> JoinedMulticastGroup;
        /// <summary>
        /// On left multicast group event handler
        /// </summary>
        public event EventHandler<IPAddress> LeftMulticastGroup;
        ...
        /// Receive Buffer
        /// </summary>
        public Buffer ReceiveBuffer { get => _receiveBuffer; set => _receiveBuffer = value; }

        public static IPAddress GetHostIPAddress(string host, AddressFamily family = AddressFamily.InterNetwork)
        {
            var ips = Dns.GetHostAddresses(host).ToList();
            return ips.Where(w => w.AddressFamily == family).FirstOrDefault();
        }
        ...
        public virtual bool Connect()
        {
            ...
            // reset stop flag so when disconnected user will know the disconnection type
            _stop = false;
        ...
        public virtual bool Disconnect()
        {
            ...
            OnDisconnected(_stop);

            return true;
        }

        public virtual void DisconnectAndStop()
        {
            _stop = true;
            Disconnect();
            while (IsConnected)
        ...
        {
            var size = Receive(ref _receiveEndpoint, _receiveBuffer.Data, 0, _receiveBuffer.Capacity);
        ...
        /// </summary>
        protected virtual void OnDisconnected(bool stop)
        {
            Disconnected?.Invoke(this, stop);
        }
        ...
        {
            JoinedMulticastGroup?.Invoke(this, address);
        }
        /// <summary>
        ...
        {
            LeftMulticastGroup?.Invoke(this, address);
        }
        ...
        {
            UdpReceivedEventArgs e = new UdpReceivedEventArgs();
            e.EndPoint = endpoint;
            e.Buffer = buffer;
            e.Offset = offset;
            e.Size = size;
        ...
        {
            UdpSentEventArgs e = new UdpSentEventArgs();
            e.EndPoint = endpoint;
            e.Sent = sent;
            Sent?.Invoke(this, e);
        }
